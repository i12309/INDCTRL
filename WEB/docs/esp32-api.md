...
7. При завершении работы ESP32 отправляет logout с текущим `sessionID`.

Работник появляется в списке и может выполнить login только если пользователь
активен, имеет право `accounts.use_esp32_api`, допущен к станку и попадает в
активное расписание работы.

Повторный login того же работника на том же станке внутри того же интервала
расписания продолжает существующую активную смену (`workID` остается тем же), но
выдает новый `sessionID`. Старые активные сессии этой смены отключаются. Если
активная смена на станке принадлежит другому работнику, login запрещается. Если
смена того же работника осталась активной с прошлого интервала расписания, она
помечается как зависшая, и создается новая смена.
...
Поле `password` опционально. Если оно передано, Django сверяет пароль с
пользователем активной сессии. ESP32 не должен выполнять повторный login перед
logout.
