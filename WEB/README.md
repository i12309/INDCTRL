...
- `POST /login/pin` (`/api/device/login` remains a compatibility alias)
...
-------------------------------------------
  ...
  Полностью остановить и удалить контейнеры, но оставить БД:
  ```docker compose down```

  Стереть все локальные данные PostgreSQL и начать заново:
