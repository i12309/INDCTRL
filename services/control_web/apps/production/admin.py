...
class DetailAdmin(ReadOnlyAdminMixin, admin.ModelAdmin):
    ...
    Детали создаются только через event-service, поэтому ручное добавление и
...
class InvalidEventAdmin(ReadOnlyAdminMixin, admin.ModelAdmin):
    ...
    Такие записи создаются auth/event-сервисами при ошибках входящих запросов, чтобы
